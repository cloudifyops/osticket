...
class Ticket {
    ...
    function load($id=0) {
        ...
        $this->fdtId    = $this->ht['fdt_id'];
        $this->policyNumber    = $this->ht['policy_number'];
        $this->mediId    = $this->ht['medi_id'];
        $this->employeeId    = $this->ht['employee_id'];
        $this->cityId   = $this->ht['city_id'];
        $this->corpName = $this->ht['corp_name'];
        $this->claimNo  = $this->ht['claim_no'];
    ...
    function getFdtId(){
        return $this->ht['fdt_id'];
    }

    function getPolicyNumber(){
        return $this->ht['policy_number'];
    }

    function getMediId(){
        return $this->ht['medi_id'];
    }

    function getEmployeeId(){
        return $this->ht['employee_id'];
    }

    function getCityId(){
        return $this->ht['city_id'];
    }

    function getLocation(){
        if($this->ht['city_id'] !=''){
            $sql = "SELECT city_name FROM ost_city WHERE city_id = ".$this->ht['city_id'];
            $res = db_query($sql);
    ...
    function getCorpName(){
        return $this->ht['corp_name'];
    }

    function getClaimNo(){
        return $this->ht['claim_no'];
    }
   ...
   function onOverdue($whine=true, $comments="") {
    ...
    function update($vars, &$errors) {
        ...
        $fields=array();
        $fields['topicId']  = array('type'=>'int',      'required'=>1, 'error'=>__('Issue Category selection is required'));
   ...
   /*============== Static functions. Use Ticket::function(params); =============nolint*/
    ...
    static function create($vars, &$errors, $origin, $autorespond=true,
        ...
        /*$fields['policyNumber']  = array('type'=>'string',  'required'=>1, 'error'=>__('Policy number required'));
        $fields['fdtId']  = array('type'=>'string',  'required'=>1, 'error'=>__('FreshDesk Ticket Id required'));
        $fields['mediId']  = array('type'=>'string',  'required'=>1, 'error'=>__('MediAssist Id required'));*/
        $fields['employeeId']  = array('type'=>'string',  'required'=>1, 'error'=>__('Employee Id required'));
        switch (strtolower($origin)) {
            case 'web':
        ...
        if ($vars['topicId']) {
            if ($topic=Topic::lookup($vars['topicId'])) {
                ...
                $errors['topicId'] = 'Invalid Issue category selected';
            }
        }
        ...
        $sql='INSERT INTO '.TICKET_TABLE.' SET created=NOW() '
            ...
            .' ,fdt_id='.db_input($vars['fdtId'])
            .' ,policy_number='.db_input($vars['policyNumber'])
            .' ,medi_id='.db_input($vars['mediId'])
            .' ,employee_id='.db_input($vars['employeeId'])
            .' ,city_id='.db_input($vars['cityId'])
            .' ,corp_name='.db_input($vars['corpName'])
            .' ,claim_no='.db_input($vars['claimNo'])
