    ...
    ?>><?php echo $desc; ?></option><?php
        ...
        <tr>
            ...
            <td>
                <span>
                <select name="city_id">
                    <option value="0" selected="selected">&mdash; <?php echo __('Select City'); ?> &mdash;</option>
                    <?php
                    $sql='SELECT city_id,city_name FROM ost_city city ORDER by city_name';
                    if(($res=db_query($sql)) && db_num_rows($res)){
                        while(list($id,$name)=db_fetch_row($res)){
                            $selected = (isset($info['city_id'])
                                    && $id == $info['city_id'])
                                ? 'selected="selected"' : '';
                            if($name)
                                $city_name=Format::htmlchars("$name");
                            echo sprintf('<option value="%d" %s>%s</option>',$id,$selected,$city_name);
                        }
                    }
                    ?>
                </select>
                &nbsp;<span class="error"><?php echo $errors['city_id']; ?></span>
                <i class="help-tip icon-question-sign" href="#auto_response_email"></i>
                </span>
            </td>
        </tr>
