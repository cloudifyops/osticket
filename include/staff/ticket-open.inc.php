...
if ($_POST){
    ...
    if($info['source'] == 'External'){
    	$extDisplay = 'style="display:table block"';
    }
}

$locationsSql = "SELECT city_id, city_name FROM ost_city";
$locationres = db_query($locationsSql);

while(list($id,$location)=db_fetch_row($locationres)){
...
<form action="tickets.php?a=open" method="post" id="save"  enctype="multipart/form-data">
 ...
 <table class="form_table fixed" width="940" border="0" cellspacing="0" cellpadding="2">
    ...
    <tr>
            ...
            <td>
                <select name="source" id="sel-source">
                    <?php
    ...
    <tbody>
        ...
        <tr class="ext-flds" <?php echo $extDisplay ?>>
            <td width="160" class="required"> <?php echo __('Corporate Name'); ?>: </td>
            <td>
                <span style="display:inline-block;">
                    <input type="text" size=45 name="corpName" id="corpName"
                        autocomplete="off" autocorrect="off" value="<?php echo $info['corpName']; ?>" /> </span>
                <font class="error">* <?php echo $errors['corpName']; ?></font>
            </td>
        </tr>

        <tr>
            <td width="160" class="required"> <?php echo __('Employee Id'); ?>: </td>
            <td>
                <span style="display:inline-block;">
                    <input type="text" size=45 name="employeeId" id="employeeId"
                        autocomplete="off" autocorrect="off" value="<?php echo $info['employeeId']; ?>" /> </span>
                <font class="error">* <?php echo $errors['employeeId']; ?></font>
            </td>
        </tr>

        <tr>
            <td width="160" > <?php echo __('Phone Number'); ?>: </td>
            <td>
                <span style="display:inline-block;">
                    <input type="text" size=45 name="phone" id="user-phone"
                        autocomplete="off" autocorrect="off" value="<?php echo $info['phone']; ?>" /> </span>
                <font class="error"> <?php echo $errors['phone']; ?></font>
            </td>
        </tr>

        <tr class="ext-flds" id="ext-medi-id" <?php echo $extDisplay ?>>
            <td width="160" > <?php echo __('MediAssist Id'); ?>: </td>
            <td>
                <span style="display:inline-block;">
                    <input type="text" size=45 name="mediId" id="mediId"
                        autocomplete="off" autocorrect="off" value="<?php echo $info['mediId']; ?>" /> </span>
                <font class="error"> <?php echo $errors['mediId']; ?></font>
            </td>
        </tr>  

        <tr class="ext-flds" id="ext-medi-id" <?php echo $extDisplay ?>>
            <td width="160" > <?php echo __('Claim Number'); ?>: </td>
            <td>
                <span style="display:inline-block;">
                    <input type="text" size=45 name="claimNo" id="claimNo"
                        autocomplete="off" autocorrect="off" value="<?php echo $info['claimNo']; ?>" /> </span>
                <font class="error"> <?php echo $errors['claimNo']; ?></font>
            </td>
        </tr>  

         <tr class="ext-flds" id="ext-policy-no" <?php echo $extDisplay ?>>
            <td width="160"> <?php echo __('Policy Number'); ?>: </td>
            <td>
                <span style="display:inline-block;">
                    <input type="text" size=45 name="policyNumber" id="policyNumber"
                        autocomplete="off" autocorrect="off" value="<?php echo $info['policyNumber']; ?>" /> </span>
                <font class="error"> <?php echo $errors['policyNumber']; ?></font>
            </td>
        </tr>

        <tr class="ext-flds" id="ext-fdt-id" <?php echo $extDisplay ?>>
            <td width="160"> <?php echo __('FreshDesk Ticket Id'); ?>: </td>
            <td>
                <span style="display:inline-block;">
                    <input type="text" size=45 name="fdtId" id="fdtId"
                        autocomplete="off" autocorrect="off" value="<?php echo $info['fdtId']; ?>" /> </span>
                <font class="error"> <?php echo $errors['fdtId']; ?></font>
            </td>
        </tr>
        ...
        <tr style="visibility:hidden">
            ...
            <td>
            <input type="checkbox" name="alertuser" ><?php //<?php echo (!$errors || $info['alertuser'])? 'checked="checked"': ''; ?>
    ...
    <tbody>
        ...
        <tr>
            <td width="160" class="required">
                <?php echo __('Issue Category'); ?>:
            </td>
            <td>
                <select id="sel-cat" name="topicId" onchange="javascript:
                    ...
                    if ($topics=Topic::getHelpTopics()) {
                        ...
                        <option value="" selected >&mdash; <?php echo __('Select Issue Category'); ?> &mdash;</option>
<?php                   }
        ...
        <tr>
            ...
            <td>
                <select name="deptId" id="sel-dept">
        ...
        <!-- Location added .. Not needed as of now -->
         <!-- <tr>
            <td width="160">
                ...
                <select name="cityId">
                    ...
                    //if($depts=Dept::getDepartments()) {
                        foreach($locations as $id =>$name) {
                            echo sprintf('<option value="%d" %s>%s</option>',
                                    $id, ($info['cityId']==$id)?'selected="selected"':'',$name);
                        }
                    //}
                    ?>
                </select>
                &nbsp;<font class="error"><?php echo $errors['cityId']; ?></font>
            </td>
        </tr> -->
       ...
       <!--  <?php
        ...
        <tr style="display: none;">
            ...
            if(($cannedResponses=Canned::getCannedResponses())) {
                ?>
                <div style="margin-top:0.3em;margin-bottom:0.5em;visibility: hidden;">
...
$(function() {
    $('#sel-source').change(function(){
        var source = $(this).val();
        if(source == 'Internal') $('.ext-flds').hide(200);
        else $('.ext-flds').show(200);
    });
    $("#sel-sla").change(function(){
         var slaId = $(this).val();
         $.ajax({
                url: "ajax.php/tickets/sla?slaId="+slaId,
                dataType: 'json',
                success: function (data) {
                    console.log(data.sla);
                    var slaDate = new Date().addHours(data.sla);
                    var slaDateStr = (slaDate.getMonth() + 1) + '/' + slaDate.getDate() + '/' +  slaDate.getFullYear();
                    console.log(slaDateStr);
                    console.log("hours "+slaDate.getHours()+ "Minutes "+slaDate.getMinutes());
                    var hr = slaDate.getHours();
                    var min = slaDate.getMinutes();
                    if(hr >=24)
                        hr=hr%24;
                    else if(hr<0)
                        hr=0;
                    ...
                    _minute=pad('00',min,1);
                    _hour=pad('00',hr,1);
                    var dueTime = _hour+":"+_minute;
                    console.log("Due Time "+dueTime);
                    $("#time").val(dueTime);
                    $("#duedate").val(slaDateStr);
                }
            });
    });
    ...
    $("#sel-cat").change(function(){
         var slaId = $(this).val();
         $.ajax({
                url: "ajax.php/tickets/dept-lookup?id="+slaId,
                dataType: 'json',
                success: function (data) {
                    console.log(data.dept_id);
                    $('#sel-dept').val(data.dept_id);
                }
            });
    });    
