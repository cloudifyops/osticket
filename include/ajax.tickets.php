...
class TicketsAjaxAPI extends AjaxController {
    ...
    #custom function to get the sla value
    function getSla(){
        $slaId = $_REQUEST['slaId'];
        if($slaId != ''){
            $sql='SELECT * FROM '.SLA_TABLE.' WHERE id='.db_input($slaId);
                if(!($res=db_query($sql)) || !db_num_rows($res))
                    return false;

                $ht=db_fetch_array($res);
                echo $this->json_encode(array('sla'=>$ht['grace_period']));
        }
    ...
    function getDeptId(){
        ...
        if($topicId !=''){
            ...
            if(!empty($topic))
                ...
                $deptId = 0;
            echo $this->json_encode(array('dept_id'=>$deptId));
        }
    }
